...
Open [http://localhost:3000](http://localhost:3000). The project runs and the contact form works end to end with zero configuration: without `RESEND_API_KEY`, the API route logs the submission to the console and returns a simulated success instead of failing.
...
| --- | --- | --- |
| `RESEND_API_KEY` | No | Resend API key. Without it, email sending is simulated and logged. |
